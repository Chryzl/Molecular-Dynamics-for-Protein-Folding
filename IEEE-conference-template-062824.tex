...
% - Motivate where the idea came from, i.e., think of training a neural network as of a protein. Where the atom's coorinates correspond to the parameters of the NN. The process of training is then similar to folding a protein, where the folding is subject to the energy landscape while model training is subject to the loss landscape
% - For basically all of Deep Learning, people wanted to get insight into the dynamics of training, particularly visualizing and interpreting the loss landscape
% - This would be highly beneficial for understanding generalization, optimization, etc.
% - And it is still not really solved how to think of the loss landscape in a meaningful way. Early on, people thought that DL training was primarily hindered by local minima, but later research showed that this is not really the case. Instead, saddle points and flatness of minima seem to be more important, where the gradient descent dynamics can get stuck or slowed down.
% - Here, we investigate if we can apply tools from MD analysis to understand the dynamics of neural network training.

\subsection{Linking Molecular Dynamics and Neural Network Training}
% It has been shown that using SGD to train neural networks is equivalent to a markovian process. The update function is: XYZ
% Furthermore, Bayesian learning, especially SGLD explicitly introduce noise into the training process, to sample from the posterior distribution of the parameters, where the dynamics follow thoes of Langevin dynamics. The update function is: XYZ
% It is not hard to see that if SGD is a markovian process, then SGLD is also a markovian process, just with added noise, which is independent of the current state, thus not breaking the markovian property.
% Additionally, by the langevin dynamics invoked by SGLD, the stationary distribution of the parameters is known to be the boltzmann distribution over the loss landscape, similar to MD simulations: Boltzmann
% Therefore, we can reasonably assume that the training of neural networks is ergodic with a known stationary distribution
% Also, given that the learning rate is constant, we set out to investigate if the training dynamics are also reversible

% These mathematical similarities raise the question of whether we can apply tools from MD analysis to understand the dynamics of neural network training. 
% In particular, we propose to view the training of neural networks through the lens of the transfer operator formalism
% In Bayesian learning, we do not aim for a single optimal set of parameters, but rather want to sample from the posterior distribution over the parameters given the data
% Therefore, we can propose the transfer operator formalism as a framework to analyze the dynamics of neural network training via defining its action on a distribution over the parameters akin to Equation \ref{eq:intro:propagator}: XYZ
% If the assumtions of markovianty, ergodicity and reversibility hold, we can then apply tools such as TICA and MSMs to analyze the training dynamics, potentially giving us not only insights into training dynamics, but a general framework to analyze neural network training akin to MD analysis

% We therefore set out to investigate if the assumptions of markovianty, ergodicity and reversibility hold for neural network training dynamics, and then to investigate training dynamics of small NNs.

\subsection{Methods}
% Short method section
% We train NN for MNIST --> 50k FFN, 1k CNN, would be the equiv. of atoms
% Bla bla the rest of all methods

\subsection{Results and Discussion}
% After training the CNN, we project the training trajectory onto the first two TICA components for lag times 1 and 4000 FIGURE X
% We can see that it formes a a parabola for tau 1 and a circle for tau 4000
% These shapes are closely related to those plotted by "TICA and Protein Folding and Random Walks"
% They find, that these shapes appear if the process underlying the dynamics is a random walk
% Therefore, we can conclude that the training dynamics of the CNN are closely related to a random walk in parameter space
% Conversely, we do not visualize the loss landscape directly and cannot conclude anything about its shape

% We can futher support this conclusion by looking at the PCA projection of the training trajectory FIGURE Y (for PC1 x PC2 and PC2 x PC3)
% Here, we see that the trajectory also forms a path and not a point cloud
% These findings align with the work of (NeurIPS paper about PCA of random walks = cosise eigenfunctions)
% In fact, our curves closely aling with Lissajous curves
% NEURIPS PAPER find that if the dimensionality of the random walk (i.e., the number of parameters of the NN) is high enough compared to the number of steps (i.e., training steps), the PCA projection will closely resemble Lissajous curves, which result from cosine like eigenfunctions

% All this strongly suggests that the observed training dynamics of the specific CNN we trained are a result of a high-dimensional random walk in parameter space, and does not give us direct insights into the loss landscape itself
% The problem appears to be solvable by either reducing the dimensionality of the NN or by increasing the number of training steps
% However, our CNN is already extraordinarily small compared to typical NNs used in practice, or even state-of-the-art large language models
% Also, in proteins, physical knowledge allows us to reduce the dimensionality of the system significantly by, e.g., only looking at backbone ca atoms. In NNs, we do not have such physical knowledge to reduce the dimensionality meaningfully
% Therefore, we can conclude that investigating training dynamics of practical NNs via MD analysis tools, like TICA and MSMs, is practically simply not feasible
% However, to the best of our knowledge, this is the first work to propose the transfer operator formalism as a framework to analyze neural network training dynamics. Despite the sobering results, we believe that this framework could enable more mathematically grounded analysis of neural network training dynamics in the future.
