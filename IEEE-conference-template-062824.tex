...
% - Despite comp advancements --> Time scale is still limited, especially since we want a statistical picture (many rare transitions) to get kinetics
% - MSMs allow to tackle that problem
% - MSMs can (bacause of markovianity) stitch together short simulations --> longer timescales
% - MSMs discretize the state space into states (by clustering the data) --> Need to balance between having many states (low discretization error) and few states (good statistics per state)
% - MSMs model how likely a transition between states is over a lag time tau --> Transition matrix, akin to propagator
% - From transition matrix --> get stationary distribution
% - And transition matrix can be gotten from "cheap" short simulations --> Even then we can get the stationary distribution!!!
% - Also MSMs do not need to define short order parameters beforehand, different to many enhanced sampling methods
% - And again, we can link MSMs to the transfer operator formalism via the variational principle using class indicator functions as basis functions
% - ==> Thus, like in QM, defining the basis set can improve the approximation

% From perez2013identification:
% - We get our states by running e.g. k-means clustering in TICA space
% - Then assign each frame to a state --> get discrete trajectory

% From noe2017collective:
% - MSMs class function can also be a gaussian --> fuzzy clustering

% From prinz2011markov:
% - CVs are nice and all, but we are always losing information + we can only do so much with them --> MSMs allow more
% - We get kinetics from MSMs via: rate theory or or based on the MD data directly
% - Also, we can do statistical error estimations
% - 
