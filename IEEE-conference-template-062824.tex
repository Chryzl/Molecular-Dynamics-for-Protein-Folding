...
\begin{abstract}
Protein folding is a complex process where a polypeptide chain navigates a rugged energy landscape to find its native structure. Molecular Dynamics (MD) simulations provide atomistic insights into this process, but analyzing the resulting high-dimensional data requires robust mathematical tools. This report reviews the transfer operator formalism, which serves as a foundation for key analysis techniques like Time-lagged Independent Component Analysis (TICA) and Markov State Models (MSMs). We explain how these methods identify slow dynamical processes and meta-stable states.

We extend this framework to deep learning, treating neural network training as a physical process on a loss landscape. Our analysis supports previous research that finds training dynamics to resemble high-dimensional random walks rather than structured folding pathways. This finding highlights that the lack of intuitive collective variables in deep learning limits the direct application of MD tools due to the high dimensionality of parameter space.
\end{abstract}
...
Lastly, we assume that the process is \textit{reversible}. Thus, the transition probability satisfies the condition of detailed balance, i.e., when at equilibrium (the stationary distribution $\mu(\mathbf{x})$), the transition rate from state $\mathbf{x}$ to $\mathbf{y}$ is equal to the reverse direction: $\mu(\mathbf{x}) p(\mathbf{x}, \mathbf{y}; \tau) = \mu(\mathbf{y}) p(\mathbf{y}, \mathbf{x}; \tau)$. The condition of detailed balance is not necessary for, e.g., Markov State Models (Section \ref{sec:msm}), but it simplifies the mathematical treatment so that we will assume it here. Generally, it is assumed that equilibrium MD simulations fulfill this condition \cite{prinz2011markov}.
...
\end{equation}
If we now consider not the probability distribution $p_t(\mathbf{x})$ directly, but rather its relative distribution with respect to the stationary distribution $\mu(\mathbf{x})$ (Equation \ref{eq:intro:prop_to_transfer}), we can define the equivalent \textit{transfer operator} $\mathcal{T}(\tau)$ as:
\begin{align}
...
The \emph{key insight} of the transfer operator formalism lies in the recognition that the eigenvalues and eigenfunctions of the propagation and transfer operators hold interpretable meanings (see Figure \ref{fig:intro:transferop}). This is most easily seen by considering the action of the propagator operator on the stationary distribution $\mu(\mathbf{x})$. Since the system is already at equilibrium, applying the propagator does not change the distribution:
\begin{equation}
    \mathcal{P}(\tau) \mu(\mathbf{x}) = \mu(\mathbf{x}) = \phi_1(\mathbf{x})
\end{equation}
Thus, the stationary distribution $\mu(\mathbf{x})$ is an eigenfunction $\phi_1(\mathbf{x})$ of the propagator operator with eigenvalue $\lambda_1 = 1$ (Figure \ref{fig:intro:transferop}c). By Equation \ref{eq:intro:prop_to_transfer}, we can see that the corresponding eigenfunction of the transfer operator is simply the constant function $\psi_1(\mathbf{x}) = \mathbf{1}$. More generally, we can define the eigenvalue problems for both operators as:
\begin{align}
...
\end{equation}
These timescales $t_i$ are also referred to as the $i$th implied timescale and measure the time a transition described by an eigenvector takes.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/transfer_operator_tau20.pdf}
    \caption{Visualization of the central concepts of the transfer operator formalism in a 1D toy potential (a, solid line). The stationary distribution is then given by the Boltzmann distribution (a, dashed line). (b) shows the first 3 eigenfunctions $\phi_i$ of the propagator operator $\mathcal{P}$, where $\phi_1$ is the stationary distribution $\mu(x)$. For example, $\phi_3$ represents the transitions between the minima at 12 and 38. (c) shows the first 3 eigenfunctions $\psi_i$ of the transfer operator $\mathcal{T}$. The figure is inspired by \cite{prinz2011markov}.}
    \label{fig:intro:transferop}
\end{figure}
...
\end{equation}
where $\hat{\psi}_i(\mathbf{x})$ is the approximation of $\psi_i(\mathbf{x})$ and $b_{ik}$ are the expansion coefficients. The optimal linear combination coefficients can be found by solving a generalized eigenvalue problem involving the covariance matrices of these basis functions at lag-time $\tau$ and 0. This allows us to approximate the eigenvalues and eigenfunctions of the transfer operator from the simulation data. Importantly, the basis functions can be chosen freely, allowing for flexibility in the approximation.
...
\end{equation}
where $\mathbf{\pi} \in \mathbb{R}^n$ is the stationary distribution over the discrete states, compared to $\mu(\mathbf{x})$ in the continuous case. As a result, we can recognize that MSMs are indeed a special case of the variational approach for approximating the eigenvalue problem of the transfer operator (Section \ref{sec:variational}) when choosing the class indicator functions defined in Equation \ref{eq:msm:class_func} as basis functions \cite{prinz2011markov,noe2017collective,sarich2010approximation}.

So far, we have only seen the general definition of the transition matrix (Equation \ref{eq:msm:trans_ij}). Practically, we must estimate the transition matrix from the discrete trajectory obtained from the MD data $\mathbf{x}(t)$ where $t$ is discretized by the simulation time-step $\delta t$. To do so, we count the number of observed transitions between all pairs of states $S_i$ and $S_j$ at a lag-time $\tau = l \cdot \delta t$. The transition probability $\mathbf{T}_{ij}(\tau)$ can then be estimated as the fraction of transitions from state $S_i$ to $S_j$ relative to the total number of transitions starting from $S_i$. This simple estimator corresponds to the maximum likelihood estimator for the transition matrix $\hat{\mathbf{T}}$ \cite{prinz2011markov}.
However, Prinz et al. \cite{prinz2011markov} also find that $\hat{\mathbf{T}}$ built from trajectory data does not necessarily fulfill detailed balance, even if the underlying dynamics are reversible. If this property is desired, there are various methods to ensure reversibility in the estimated transition matrix \cite{prinz2011markov}.
...
\begin{equation}
    \theta_{t+1} = \theta_t - \eta \nabla L(\theta_t; \mathcal{B}_t) \label{eq:sgd}
\end{equation}
where $\theta_t$ are the parameters at time step $t$, $\eta$ is the learning rate, $\nabla L(\theta_t; \mathcal{B}_t)$ is the gradient of the loss function computed on a mini-batch $\mathcal{B}_t$ \cite{amari1993backpropagation}. To draw the parallel to MD simulations, the SGD update rule would thus be akin to an integration scheme for simulating the dynamics of a particle in a potential energy landscape.
...
\begin{equation}
    \theta_{t+1} = \theta_t - \eta \nabla L(\theta_t; \mathcal{B}_t) + \sqrt{2 \eta} \cdot \epsilon_t \; \text{with} \; \epsilon_t \sim \mathcal{N}(0, I) \label{eq:sgld}
\end{equation}
...
Given that SGD with a constant learning rate is a Markovian process \cite{mandt2017stochastic}, constant SGLD is also Markovian, as the added noise is independent of the current state and does not break the Markovian property. Additionally, following Prinz et al. \cite{prinz2011markov}, we can reasonably assume that NN training via SGLD is sufficiently ergodic since it gives rise to a defined stationary distribution. Lastly, it remains open if the training dynamics are reversible. Showing it is beyond the scope of this report, and we will simply assume reversibility.
...
\end{equation}
where $p(\theta, t)$ is the distribution over the parameters $\theta$ at time $t$. If the assumptions of Markovianity, ergodicity, and reversibility hold, we can then apply tools such as TICA and MSMs to analyze the training dynamics, potentially providing insights into training dynamics and establishing a general framework to analyze neural network training akin to MD analysis.
...
\begin{itemize}
    \item \textbf{Minimization (Phase 1):} The models were first trained using regular SGD defined in Equation \ref{eq:sgd} for 20 epochs to reach a local minimum in the loss landscape. This corresponds to the energy minimization step in MD.
    \item \textbf{Equilibration (Phase 2):} We then switched to the Stochastic Gradient Langevin Dynamics (SGLD) optimizer defined in Equation \ref{eq:sgld_final}. The system was evolved for 10,000 steps to allow the parameters to equilibrate to the stationary distribution defined by the noise scale.
...
\begin{equation}
    \theta_{t+1} = \theta_t - \eta \nabla L(\theta_t) + \sqrt{2\eta} \sigma \epsilon \label{eq:sgld_final}
\end{equation}
...
If we look at the TICA projection onto the first 2 components for the FFN and CNN training trajectories (Figure \ref{fig:dl:tica}), we observe that both trajectories form distinct paths rather than point clouds, which we would expect if we sampled the loss-landscape. Specifically, we see that both trajectories exhibit a clear parabolic shape at lag-time $\tau=10$ and a circular shape at lag-time $\tau=4000$. However, it is also visible that the smaller CNN model's projections show a broader path traced out in the TICA projection compared to the FFN.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/tica_comparison.pdf}
    \caption{Comparison of TICA projections for CNN (top) and FFN (bottom) training trajectories. The projections show the first two independent components (IC1 vs IC2) at different lag times. The parabolic and circular shapes are characteristic of high-dimensional random walks \cite{schultze2021time}.}
    \label{fig:dl:tica}
\end{figure}

These observed shapes in the TICA projections are very similar to those reported by Schultze et al. \cite{schultze2021time} in their study of TICA applied to random walks in the context of protein folding. They find that molecular dynamics simulations that have not converged to the stationary distribution or sampled the state space sufficiently exhibit TICA projections that resemble those of random walks. Therefore, we can conclude that the training dynamics of both the FFN and CNN are much more closely related to a high-dimensional random walk in parameter space rather than a meaningful exploration of the loss landscape itself, at least within the timescales we simulated.

These findings are further supported by the PCA projections of the training trajectories (Figure \ref{fig:dl:pca}). Similar to the TICA projections, we observe that both the FFN and CNN training trajectories form distinct paths in the PCA space, which aligns with findings from Antognini et al. \cite{antognini2018pca}. They show that PCA projections of high-dimensional random walks result in Lissajous curves due to the cosine-like nature of the eigenfunctions. They show this explicitly if the dimensionality of the system is much larger than the number of time steps. However, even for the CNN with only 1k parameters but 80k saved steps, we observe similar behavior, indicating that the training dynamics are dominated by random-walk-like behavior in parameter space.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figures/pca_comparison.pdf}
    \caption{PCA projections of the training trajectories for CNN (left) and FFN (right). The trajectories form distinct paths resembling Lissajous curves, which is indicative of high-dimensional random walks \cite{antognini2018pca}.}
    \label{fig:dl:pca}
\end{figure}

The resemblance of our projected trajectories to high-dimensional random walks suggests that the system primarily underwent diffusive motion on the simulated timescales, rather than discrete transitions between meta-stable basins. Crucially, this was not due to excessive noise, as both models maintained high test accuracy ($>90\%$), indicating confinement within a valid solution manifold. Instead, the lack of structured transitions likely stems from the immense volume of the parameter space (even for the tiny CNN) requiring significantly longer sampling to observe rare events.

To the best of our knowledge, this work is the first to propose the transfer operator formalism as a framework for analyzing neural network training dynamics. Practical application is currently limited by the high dimensionality of NNs. In protein folding, systems with thousands of atoms can be effectively reduced to a few degrees of freedom by e.g. only considering $C_\alpha$ positions, thus reducing the redundancy while keeping most structural information. In contrast, even for our tiny CNN with only $\approx 1000$ parameters, no such intuitive collective variables exist to filter out the high-dimensional noise. However, the transfer operator formalism could provide a solid mathematical foundation for future work on investigating NN training dynamics theoretically.

\section*{Code Availability}
The code and data to reproduce the experiments and figures in this report are available at \url{https://github.com/Chryzl/Molecular-Dynamics-for-Protein-Folding}.

\section*{Acknowledgments}
Generative AI was used to improve the language and grammar of this report as well as support the implementation of the figures and training code.
